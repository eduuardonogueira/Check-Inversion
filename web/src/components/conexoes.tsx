...
export function Conexoes({consulta}: IConexoes): JSX.Element {
    return (
        <div className="flex flex-col gap-1 rounded w-full">
            {consulta.map((item, index) => (
                <li 
                    className="flex h-min-[0px]" 
                    key={index}
                    onClick={''}    
                >
                    <span className="bg-[#1E1E1E] text-white w-[50px] font-bold box-border flex items-center justify-center">{index + 1}</span>

                    <div className="bg-[#11346B] hover:bg-[#044FBF] flex gap-6 px-6 py-2 h-full w-full text-white items-center justify-between">
                        <ul className="flex gap-3 w-max-[35%]">
